...
export const metadata = createPageMetadata({
    title: "Admin - Remote Jobs",
    description: "Manage remote jobs.",
    path: "/admin/remote-jobs",
});

export default async function AdminRemoteJobsPage() {
    const jobs = (await fetchActiveJobs()).filter((job) =>
        job.categories.includes("Remote")
    );
    ...
    async function deleteAction(formData: FormData) {
        ...
        if (slug) {
            await removeJobCategory(slug, "Remote");
            revalidatePath("/admin/remote-jobs");
        ...
        <div className="pb-16 pt-8 lg:pt-10">
            <ShellContainer className="space-y-10">
                <section className="space-y-4">
                    ...
                    <h1 className="jobs4u-heading text-2xl font-semibold tracking-tight text-slate-950 sm:text-3xl ">Manage Remote Jobs</h1>
                </section>
                ...
                <section className="space-y-6">
                    ...
                    {jobs.length ? (
                        ...
                        <EmptyState title="No remote jobs found" description="Publish verified remote jobs from the admin dashboard to populate this section." />
                    )}
                </section>
            </ShellContainer>
        </div>
